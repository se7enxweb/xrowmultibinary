...
(function() {
    ...
    var plupload = {
        ...
        /**
         * Number of files error. If the user selects two much files it will be blocked and an error of this type will be triggered.
         *
         * @property FILE_NUMBER_ERROR
         * @final
         */
        FILE_NUMBER_ERROR : -610,
        ...
        plupload.extend(this, {
            ...
            init : function() {
                ...
                self.bind('FilesAdded', function(up, selected_files) {
                    var i, file, count = 0, extensionsMap, filters = settings.filters, alert_was_displayed = 0;
                    ...
                    for (i = 0; i < selected_files.length; i++) {
                        ...
                        if ( settings.no_files_with_same_name )
                        {
                            $.each( files, function( i, existingfile ) {
                                if ( existingfile.name == file.name )
                                {
                                    ...
                                    alert( plupload.translate( 'A file with this name is already exists.' ) );
                                }
                            });
                        }
                        ...
                        //alert('check_max_number_of_files ' + check_max_number_of_files + ' count ' + count + ' max_number_of_files '+ max_number_of_files);
                        if ( check_max_number_of_files == 'enabled' && count >= max_number_of_files || files.length >= max_number_of_files )
                        {
                            var error = plupload.translate( 'You are allowed to upload only ' ) + max_number_of_files + plupload.translate( ' files.' );
                            if ( max_number_of_files == 1 )
                            {
                                error = plupload.translate( 'You are allowed to upload only one file.' );
                            }
                            ...
                            up.trigger('Error', {
                                code : plupload.FILE_NUMBER_ERROR,
                                message : 'Number of files error.',
                                file : file
                            });
